...
lattice_beam=0.01  # Beam we use in lattice generation.

#. parse_options.sh || exit 1;

lat_wspecifier="ark:|lattice-scale --acoustic-scale=$post_decode_acwt ark:- ark:- | gzip -c >lat.gz"

symbols=data/lang/chars_new_symbol_table.txt
...
latgen-faster --word-symbol-table=$symbols \
  ...
  data/lang_mixed_0.8/TLG_det.fst ark:logits.ark "$lat_wspecifier"
...
lattice-scale --acoustic-scale=10 --lm-scale=10 "ark:gunzip -c lat.gz|" ark:- | lattice-add-penalty \
  ...
  --word-symbol-table=data/lang/chars_new_symbol_table.txt ark:- ark,t: | int2sym.pl -f 2- $symbols >${hyps_file_tmp}
