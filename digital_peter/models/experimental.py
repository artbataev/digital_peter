...
from digital_peter.models.conv import ConvExtractor, ResnetExtractor, ConvExtractorDeeper, ConvExtractorHiRes
...
def resnet__gru_2x368_drop02(num_outputs) -> nn.Module:
    model = SequentialSequential(*[
        ResnetExtractor(),
        LambdaModule(lambda seq, seq_len: (F.relu(seq), seq_len)),
        RNNEncoder(dropout=0.2, rnn_type="GRU", num_layers=2, hidden_size=368, input_size=512),
...
def conv_deeper__gru_2x256_drop02(num_outputs) -> nn.Module:
    model = SequentialSequential(*[
        ConvExtractorDeeper(),
        LambdaModule(lambda seq, seq_len: (F.relu(seq), seq_len)),
        RNNEncoder(dropout=0.2, rnn_type="GRU", num_layers=2, hidden_size=256, input_size=512),
...
def conv_deeper__gru_2x368_drop02(num_outputs) -> nn.Module:
    model = SequentialSequential(*[
        ConvExtractorDeeper(),
        LambdaModule(lambda seq, seq_len: (F.relu(seq), seq_len)),
        RNNEncoder(dropout=0.2, rnn_type="GRU", num_layers=2, hidden_size=368, input_size=512),
...
def conv_hires__gru_2x256_drop02(num_outputs) -> nn.Module:
    model = SequentialSequential(*[
        ConvExtractorHiRes(),
        LambdaModule(lambda seq, seq_len: (F.relu(seq), seq_len)),
        RNNEncoder(dropout=0.2, rnn_type="GRU", num_layers=2, hidden_size=256, input_size=512),
...
def conv_hires__gru_2x368_drop02(num_outputs) -> nn.Module:
    model = SequentialSequential(*[
        ConvExtractorHiRes(),
        LambdaModule(lambda seq, seq_len: (F.relu(seq), seq_len)),
        RNNEncoder(dropout=0.2, rnn_type="GRU", num_layers=2, hidden_size=368, input_size=512),
